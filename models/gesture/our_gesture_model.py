...
class OurGestureModel(UpstreamModel):
    def __init__(self, model_path="ourModelsprojects/gesture/checkpoints/model_v1.pth", device=None):
        if device is None:
        ...
        try:
            ...
            sys.path.insert(0, os.path.abspath("ourModelsprojects/gesture"))
            try:
                from models.custom_gesture import get_model
            ...
            if os.path.exists(model_path):
                ...
                print(f"[OurGestureModel] Loaded weights from {model_path}")
            else:
                print(f"[OurGestureModel] Weights not found at {model_path}. Using uninitialized weights.")
        except Exception as e:
            print(f"[OurGestureModel] Error initializing model: {e}")
    ...
    def get_name(self) -> str:
        return "Our Gesture Model (Custom)"
