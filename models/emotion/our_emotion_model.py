...
from mcn.config import EMOTION_CATEGORIES

class OurEmotionModel(UpstreamModel):
    def __init__(self, model_path="ourModelsprojects/emotion/checkpoints/model_v1.pth", device=None):
        if device is None:
        ...
        try:
            ...
            sys.path.insert(0, os.path.abspath("ourModelsprojects/emotion"))
            try:
            ...
            if os.path.exists(model_path):
                ...
                print(f"[OurEmotionModel] Loaded weights from {model_path}")
            else:
                print(f"[OurEmotionModel] Weights not found at {model_path}. Using uninitialized weights.")
        except Exception as e:
            print(f"[OurEmotionModel] Error initializing model: {e}")
    ...
    def get_name(self) -> str:
        return "Our Emotion Model (Custom)"
